...
namespace TransportWorkshopCore.Data
{
    ...
    {
        public static void Initialize(TransportWorkshopCoreContext context)
        {
            ...
            string[] nameFuel = { "ДТ", "ДТ-кз", "А92", "А95" };                                                                                                        //типы топливо 4
            ...
            string[] descriptionsTypeAuto = {"Внутриреспубликанский грузовой", "Служебный(специальный)легковой", "Внутриреспубликанский автобус",
            ...
            for (int i = 0; i < typeOfworkNumber; i++)      //Норма топлива +
            {
                context.NormaFuels.Add(new NormaFuel
                {
                   ...
                   // Winter = Convert.ToInt32(random.Next(7, 11))
                    Linear = random.NextDouble() * (random.Next(6, 9)),  //десятые доли
                    Summer = random.NextDouble() * (random.Next(5, 7)),
                    Winter = random.NextDouble() * (random.Next(7, 11))
                }) ;
            }
            context.SaveChanges();
